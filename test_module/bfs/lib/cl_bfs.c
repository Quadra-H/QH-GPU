...
void _clInit(cl_context ctx, cl_device_id* devices) {
	...
	oclHandles.program = clCreateProgramWithSource(oclHandles.context, 1, (const char**) &source, &sourceSize, &resultCL);
	...
	resultCL = clBuildProgram(oclHandles.program, /*num of device*/1, oclHandles.devices, /*option*/NULL, /*pfn_notify?*/NULL, NULL);
...
void _clRelease() {
	...
	if (errorFlag)
		printf("ReleaseCL()::Error encountered..\n");
}
...
void _clSetArgs_1(int kernel_id, int arg_idx, void * d_mem, int size) {
	oclHandles.cl_status = clSetKernelArg(oclHandles.kernels[kernel_id],
			arg_idx, size, d_mem);
	if (oclHandles.cl_status != CL_SUCCESS) {
...
void _clInvokeKernel(int kernel_id, int work_items, int work_group_size) {
	...
	//printf("work_group_size : %d , work_items: %d \n", work_group_size, work_items);
	size_t local_work_size[] = { 128, 1 };
	size_t global_work_size[] = { 2048, 1 };
...
void _clInvokeKernel2D(int kernel_id, int range_x, int range_y, int group_x, int group_y) {
	...
	//work_items = work_items + (work_group_size-(work_items%work_group_size));
	oclHandles.cl_status = clEnqueueNDRangeKernel(oclHandles.queue, oclHandles.kernels[kernel_id], work_dim, 0, global_work_size, local_work_size, 0, 0, &(e[0]));
