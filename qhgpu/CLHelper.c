/*
CLHelper는 OpenCL 토대를 만든 Init함수를 비롯하여,
OpenCL API를 사용할 때 하는 예외 상황을 처리해준 것.
*/
...
int total_kernels = 1;
string kernel_names[1] = { "vector_add" }; //수정

char kernel_file[100] = "vector_add_kernel.cl"; //수정
...
void _clInit()
{
	...
	targetPlatform = allPlatforms[0]; //target platform을 첫 번째 platfrom으로 선택. default값.
	...
	//두 번째, OpenCL Device 찾기. - 현재는 Device type이 GPU로 설정. 바꾸어야할 부분. CPU에서도 가능할 수 있게.
